...
const HERO_PRODUCTS = [
  { name: 'برجر كلاسيك', price: '125', image: 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=700&auto=format&fit=crop&q=82' },
  { name: 'باستا كريمي', price: '155', image: 'https://images.unsplash.com/photo-1473093295043-cdd812d0e601?w=700&auto=format&fit=crop&q=82' },
  { name: 'قهوة باردة', price: '85', image: 'https://images.unsplash.com/photo-1509042239860-f550ce710b93?w=700&auto=format&fit=crop&q=82' },
  { name: 'تشيز كيك', price: '95', image: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=700&auto=format&fit=crop&q=82' },
]
...
const businessTypes = [
  {
    ...
    text: 'منيو بصري سريع وواضح للأطباق والمشروبات.',
    image: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=1200&auto=format&fit=crop&q=82',
  },
  {
    icon: Store,
    title: 'سوبر ماركت',
    text: 'كتالوج منتجات جاهز، العميل يختار المنتج ويضيف السعر.',
    image: 'https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=1200&auto=format&fit=crop&q=82',
  },
  {
    icon: Sparkles,
    title: 'مستحضرات تجميل',
    text: 'عرض مناسب للبراندات والصور والدرجات والأحجام.',
    image: 'https://images.unsplash.com/photo-1596462502278-27bfdc403348?w=1200&auto=format&fit=crop&q=82',
  },
]
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="relative w-12 h-12 rounded-[18px] bg-gradient-to-br from-[#f2d38e] via-[#d7b66f] to-[#82683c] shadow-[0_14px_35px_rgba(215,182,111,.28)] border border-white/25 flex items-center justify-center overflow-hidden">
        ...
        <span className="absolute -left-1 -bottom-1 w-6 h-6 rounded-full bg-[#171714] border-2 border-[#d7b66f] flex items-center justify-center">
          <Utensils size={11} className="text-[#f4e2bb]" />
        </span>
      </div>
      {!compact && (
        <div className="leading-none">
          <div className="font-display font-black text-[19px] tracking-tight text-white">Egy Menu</div>
          <div className="text-[9px] uppercase tracking-[0.2em] text-[#d7b66f]/80 mt-1.5">Smart QR Menu</div>
        </div>
      )}
    </div>
  )
...
function GlassCard({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <div className={`border border-white/10 bg-white/[0.055] backdrop-blur-xl shadow-[0_24px_70px_rgba(0,0,0,.28)] ${className}`}>{children}</div>
}
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#10110f]/90 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-5 sm:px-7 py-3.5 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-7">
            {NAV_LINKS.map((item) => <a key={item.href} href={item.href} className="text-sm text-white/60 hover:text-white transition-colors">{item.label}</a>)}
          </nav>
      ...
      <main>
        <section className="relative bg-[#10110f] text-white min-h-[760px] flex items-center">
          ...
          <div className="relative max-w-7xl mx-auto px-5 sm:px-7 py-20 lg:py-28 w-full grid lg:grid-cols-[1fr_520px] gap-14 items-center">
            <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <span className="inline-flex items-center gap-2 rounded-full border border-[#d7b66f]/25 bg-[#d7b66f]/10 text-[#ead19a] px-4 py-2 text-sm mb-6"><Sparkles size={15} /> تجربة 10 أيام + تصميمات 3D</span>
              ...
              <div className="grid grid-cols-3 max-w-xl gap-3 mt-10">
                {[["10 أيام", "تجربة مجانية"], ["3D", "قوالب وأشكال"], ["24/7", "منيو متاح"]].map(([value, label]) => (
                  <div key={label} className="rounded-2xl border border-white/10 bg-white/[0.045] px-3 py-4 text-center"><div className="font-display text-xl sm:text-2xl font-bold text-[#ead19a]">{value}</div><div className="text-[11px] sm:text-xs text-white/45 mt-1">{label}</div></div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.94, rotateY: -8 }} animate={{ opacity: 1, scale: 1, rotateY: 0 }} transition={{ duration: 0.75, delay: 0.15 }} className="relative [perspective:1200px]">
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {HERO_PRODUCTS.map((product, i) => (
                        <div key={product.name} className="rounded-[24px] bg-white p-3 border border-black/5 shadow-[0_15px_30px_rgba(0,0,0,.09)]" style={{ transform: `perspective(600px) rotateY(${i % 2 ? -2 : 2}deg)` }}>
                          <div className="aspect-[4/3] rounded-[18px] overflow-hidden bg-[#eee7dc]"><img src={product.image} alt={product.name} className="w-full h-full object-cover" loading="lazy" /></div>
                          <div className="mt-3 font-semibold text-sm">{product.name}</div>
                          <div className="flex items-center justify-between mt-2"><span className="font-bold">{product.price} ج.م</span><span className="w-7 h-7 rounded-full bg-[#d7b66f] flex items-center justify-center font-bold">+</span></div>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <GlassCard className="absolute -left-4 sm:-left-10 top-16 rounded-2xl p-4 text-white w-40 [transform:translateZ(40px)]"><div className="text-[#ead19a] text-xs">القالب</div><div className="font-bold mt-1">3D فاخر</div><div className="text-[11px] text-white/45 mt-2">صور + عمق + ظل</div></GlassCard>
              <GlassCard className="absolute -right-3 sm:-right-8 bottom-16 rounded-2xl p-4 text-white w-44 [transform:translateZ(50px)]"><div className="flex items-center gap-2"><CheckCircle2 size={17} className="text-[#9eaa83]" /><span className="font-semibold text-sm">مناسب للموبايل</span></div><div className="text-[11px] text-white/45 mt-2">تصميم واضح وسريع في الطلب</div></GlassCard>
            </motion.div>
          </div>
        </section>
        ...
        <section className="bg-[#efe9df] py-14 border-b border-black/5">
          <div className="max-w-7xl mx-auto px-5 sm:px-7">
            <div className="text-center mb-8"><p className="text-sm text-[#756d61]">مناسب لأنواع أنشطة مختلفة</p><h2 className="font-display text-2xl sm:text-3xl font-bold mt-2">نفس المنصة، شكل مناسب لكل نشاط</h2></div>
            <div className="grid md:grid-cols-3 gap-5">
              {businessTypes.map((item, i) => (
                <motion.div key={item.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group rounded-[30px] bg-[#f8f4ed] border border-black/5 overflow-hidden shadow-[0_24px_55px_rgba(53,45,34,.09)] hover:-translate-y-1.5 transition-transform">
                  <div className="relative h-52 overflow-hidden"><img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-[1.04] transition-transform duration-500" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-t from-black/65 via-black/10 to-transparent" /><div className="absolute right-5 bottom-4 w-12 h-12 rounded-2xl bg-[#171714]/90 text-[#d7b66f] flex items-center justify-center shadow-lg"><item.icon size={22} /></div></div>
                  <div className="p-6"><h3 className="font-display font-bold text-lg">{item.title}</h3><p className="text-sm text-[#776f63] leading-6 mt-2">{item.text}</p></div>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section id="menus" className="py-18 bg-[#151612] text-white scroll-mt-20">
          <div className="max-w-7xl mx-auto px-5 sm:px-7 pt-16"><div className="max-w-2xl mb-9"><span className="text-[#d7b66f] text-sm font-semibold">نماذج حقيقية للشكل</span><h2 className="font-display text-3xl sm:text-4xl font-bold mt-2">شوف المنيو قبل ما تختار شكله</h2><p className="text-white/50 mt-3 leading-7">اختار شكل العرض اللي يناسب نشاطك، وبعد التسجيل تقدر تغيّره في أي وقت.</p></div><div className="pb-14"><FeaturedMenus /></div></div>
        </section>
        ...
        <section id="features" className="py-20 bg-[#f7f2ea] scroll-mt-20">
          <div className="max-w-7xl mx-auto px-5 sm:px-7">
            <div className="text-center max-w-2xl mx-auto mb-12"><span className="text-[#8d7444] text-sm font-semibold">كل حاجة من مكان واحد</span><h2 className="font-display text-3xl sm:text-4xl font-bold mt-2">شكل احترافي وتشغيل بسيط</h2></div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">{features.map((feature, i) => <motion.div key={feature.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="rounded-[26px] bg-white p-6 border border-black/5 shadow-[0_18px_40px_rgba(0,0,0,.06)]"><div className="w-11 h-11 rounded-2xl bg-[#ece5d9] text-[#657052] flex items-center justify-center mb-4"><feature.icon size={21} /></div><h3 className="font-display font-bold">{feature.title}</h3><p className="text-sm text-[#776f63] leading-6 mt-2">{feature.text}</p></motion.div>)}</div>
          </div>
        </section>
        ...
        <section id="pricing" className="py-20 bg-[#11120f] text-white scroll-mt-20">
          <div className="max-w-7xl mx-auto px-5 sm:px-7">
            <div className="text-center max-w-2xl mx-auto mb-12"><span className="text-[#d7b66f] text-sm font-semibold">اشتراك مرن</span><h2 className="font-display text-3xl sm:text-4xl font-bold mt-2">اختار المدة المناسبة لنشاطك</h2><p className="text-white/45 mt-3">ابدأ بالتجربة المجانية وبعدها جدّد بالمدة اللي تناسبك.</p></div>
        ...
        <section className="bg-[#11120f] text-white py-16"><div className="max-w-5xl mx-auto px-5 sm:px-7 text-center"><div className="rounded-[36px] border border-white/10 bg-gradient-to-br from-[#25271f] to-[#121310] p-8 sm:p-12 shadow-[0_35px_80px_rgba(0,0,0,.35)]"><div className="flex justify-center mb-6"><BrandLogo compact /></div><h2 className="font-display text-3xl sm:text-4xl font-bold">ابدأ منيوك بشكل يليق بنشاطك</h2><p className="text-white/50 mt-3">سجّل وجرّب 10 أيام، وبعدها اختار الشكل والمدة اللي يناسبوك.</p><Link to="/register" className="inline-flex mt-7 rounded-2xl bg-[#d7b66f] text-[#171714] px-7 py-3.5 font-bold items-center gap-2">ابدأ الآن <ArrowLeft size={18} /></Link></div></div></section>
      </main>

      <footer className="bg-[#0b0c0a] text-white/45 border-t border-white/5"><div className="max-w-7xl mx-auto px-5 sm:px-7 py-7 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm"><BrandLogo /><div>منيو إلكتروني ذكي لكل نشاط</div></div></footer>
    </div>
  )
}
