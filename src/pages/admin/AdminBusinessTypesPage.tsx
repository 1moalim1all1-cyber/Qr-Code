...
export default function AdminBusinessTypesPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    setError(null)
    try {
      await createBusinessType({ name: form.name, icon: form.icon, description: form.description, sortOrder: form.sortOrder ? Number(form.sortOrder) : undefined })
  ...
  async function toggle(item: BusinessTypeRecord) {
    const message = item.is_active ? `إخفاء «${item.name}» من الاختيارات؟\nالمحلات المرتبطة به لن تُحذف أو تتأثر.` : `إعادة تفعيل «${item.name}»؟`
    ...
    <div className="min-h-screen bg-[#f7f1e8]" dir="rtl">
      <header className="bg-[#10110f] text-white border-b border-white/10">
        <div className="max-w-6xl mx-auto px-5 py-5 flex items-center gap-4">
          ...
          <div><p className="text-xs text-white/45">إعدادات المنصة</p><h1 className="font-display text-xl font-bold flex items-center gap-2"><Tags size={20} className="text-saffron" /> أنواع الأنشطة</h1></div>
        </div>
      </header>
      <main className="max-w-6xl mx-auto px-5 py-7">
        <div className="rounded-3xl bg-white border border-black/5 shadow-sm p-5 mb-6">
          <div className="mb-4"><h2 className="font-display text-lg font-bold">إضافة نوع نشاط جديد</h2><p className="text-sm text-stone mt-1">بعد إضافته هيظهر تلقائيًا في اختيارات إضافة المحلات، من غير أي تعديل في الكود.</p></div>
        ...
        <div className="rounded-3xl bg-white border border-black/5 overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-black/5 flex items-center justify-between"><div><h2 className="font-display font-bold">كل أنواع الأنشطة</h2><p className="text-xs text-stone mt-1">الحذف هنا آمن: النوع بيتوقف فقط، والمحلات القديمة تفضل شغالة.</p></div><span className="rounded-full bg-paper-dim px-3 py-1 text-xs text-stone">{items.length} نوع</span></div>
          {loading ? <div className="py-16 flex justify-center gap-2 text-stone"><LoaderCircle className="animate-spin" size={18} /> جارِ التحميل...</div> : <div className="divide-y divide-black/5">{items.map((item) => <div key={item.id} className="p-4 sm:p-5 flex items-center gap-4"><div className="w-12 h-12 rounded-2xl bg-[#f3ede3] flex items-center justify-center text-2xl shrink-0">{item.icon || '🏪'}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2 flex-wrap"><p className="font-bold">{item.name}</p><span className={`text-[10px] rounded-full px-2 py-0.5 ${item.is_active ? 'bg-green-50 text-green-700' : 'bg-stone-100 text-stone-500'}`}>{item.is_active ? 'مفعّل' : 'مخفي'}</span></div><p className="text-xs text-stone mt-1 truncate">{item.description || `الكود: ${item.code}`}</p></div><div className="flex items-center gap-2"><button onClick={() => edit(item)} className="rounded-xl border border-black/10 p-2.5 hover:bg-paper-dim" title="تعديل"><Pencil size={16} /></button><button onClick={() => toggle(item)} className={`rounded-xl p-2.5 ${item.is_active ? 'border border-red-100 text-red-600 hover:bg-red-50' : 'border border-green-100 text-green-700 hover:bg-green-50'}`} title={item.is_active ? 'حذف/إخفاء' : 'استرجاع'}>{item.is_active ? <Trash2 size={16} /> : <RotateCcw size={16} />}</button></div></div>)}</div>}
        </div>
      </main>
    </div>
  )
